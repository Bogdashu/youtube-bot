...
LOCAL_BOT_API_URL = (os.getenv("LOCAL_BOT_API_URL") or "").rstrip("/")
...
def estimate_size_mb(url, height):
    try:
        cmd = [
            ...
            "-f", f"(bv*[height<={height}]+ba/b)",
            "--print", "%(filesize_approx)s", url
        ]
        out = subprocess.check_output(cmd, text=True, stderr=subprocess.DEVNULL, timeout=120).strip()
...
def get_real_resolution(filepath):
    try:
        cmd = ["ffprobe", "-v", "error", "-select_streams", "v:0", "-show_entries", "stream=height", "-of", "csv=p=0", filepath]
...
# === ОТПРАВКА ВИДЕО ===
async def send_video_file(update, video_file, caption, size_mb, local_app):
    with open(video_file, "rb") as video:
        if size_mb <= 49 or local_app is None:
            ...
            await update.message.reply_video(
                video=video, caption=caption, supports_streaming=True,
                read_timeout=1200, write_timeout=1200
            )
        else:
            await local_app.bot.send_video(
                chat_id=update.effective_chat.id, video=video, caption=caption,
                supports_streaming=True, read_timeout=1200, write_timeout=1200
            )

# === РАБОТА С REMOTE WORKER ===
async def run_remote_job(update, msg, url, target_height):
    if not WORKER_SECRET:
        await safe_edit(msg, "WORKER_SECRET не задан в переменных.")
        return True

    headers = {"X-Worker-Secret": WORKER_SECRET}
    payload = {"url": url, "chat_id": update.effective_chat.id, "target_height": target_height}
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            ...
            while True:
                ...
                if state == "queued":
                    ...
                    text = f"📤 Отправка видео через Telegram...{size_text}"
                elif state == "done":
                    ...
                    text = f"ℹ️ Статус: {state}"

                if text != last_text:
                    await safe_edit(msg, text)
                    last_text = text
    except Exception as exc:
        await safe_edit(msg, f"⚠️ Не удалось выполнить задачу на сервере:\n\n{exc}")
        return False

# === ЛОКАЛЬНОЕ СКАЧИВАНИЕ (запасной вариант) ===
async def download_local_and_send(update, msg, url, target_height):
    await safe_edit(msg, f"📥 Скачивание на Railway...\n\n🎞 Цель: {target_height}p\n⏳ 0%")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        cmd = [
            ...
            "--extractor-args", "youtube:player_client=android_vr,web",
            "-N", "4",
            "-f", f"(bv*[height<={target_height}]+ba/b)/(bv*[height<=720]+ba/b)/best",
            "--merge-output-format", "mp4", "--newline", "-o", outtmpl, url
        ]
        ...
        if return_code != 0:
            await safe_edit(msg, "❌ Ошибка yt-dlp")
            return False
        ...
        if not video_file:
            await safe_edit(msg, "❌ Видео не найдено")
            return False

        real_quality = get_real_resolution(video_file)
        size_mb = os.path.getsize(video_file) / 1024 / 1024

        await safe_edit(msg, f"📤 Отправка видео...\n\n🎞 {real_quality}\n📦 {size_mb:.1f} MB")
        caption = f"✅ Готово\n🎞 {real_quality}\n📦 {size_mb:.1f} MB"
        
        local_app = LOCAL_APP if LOCAL_BOT_API_URL else None
        await send_video_file(update, video_file, caption, size_mb, local_app)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if should_use_worker(estimated_mb):
        ...
        if ok:
            return
        await safe_edit(msg, "⚠️ Сервер не справился. Пробую Railway fallback...")
