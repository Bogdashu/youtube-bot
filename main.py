    ...
    with open(path, "rb") as fh:
        if mode == "audio":
            ...
            await bot.send_video(chat_id=chat_id, video=fh, caption=cap,
                                 supports_streaming=True, read_timeout=1800, write_timeout=1800)
...
async def on_worker(q, url, mode, size_mb):
    ...
    await q.edit_message_text(f"📤 Заливаю в Telegram (~{size_mb:.0f} MB)...")
    ...
    if mode == "audio":
        ...
        await LOCAL_APP.bot.send_video(chat_id=chat_id, video=file_url, caption=cap,
                                       supports_streaming=True, read_timeout=2400, write_timeout=2400)
